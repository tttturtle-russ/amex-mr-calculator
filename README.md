...
原生 HTML/CSS/JS，无构建、无运行时依赖。字体（Fraunces / Space Mono）已打包至 `fonts/` 本地托管，不依赖 Google Fonts CDN；中文标题用系统衬线（Songti/宋体）回退。本地双击即可打开（`file://` 下浏览器可能拦截实时汇率请求与本地字体，属正常；线上 https 一切正常）。
